...
export default function MainInterface() {
  ...
  const { isConnected, connectedDevices, messages, sendMessage, registerDevice, emergencyAlert, clearEmergencyAlert } =
    useWebSocket()
  ...
  const handleEmergencyActivate = useCallback(() => {
    const emergencyText = "🚨 EMERGENCY ALERT - IMMEDIATE ASSISTANCE REQUIRED IN ENGINE ROOM 🚨"
    sendMessage({
      text: emergencyText,
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <EmergencyAlert isActive={!!emergencyAlert} onStop={handleStopEmergency} />

      {/* Engine Room Flashing LED */}
      <EngineRoomFlashingLED isEmergency={!!emergencyAlert} />

      {/* Header */}
      <div className="bg-gray-800 border-b-4 border-green-400 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <h1 className="text-2xl font-bold text-green-400 tracking-wide">SOL9X INDUSTRIAL COMMUNICATION SYSTEM</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[calc(100vh-140px)]">
        ...
        <div className="bg-gray-900 border-r-4 border-green-400 p-6">
          <div className="h-full flex flex-col">
            <div className="flex items-center gap-3 mb-6">
              <Radio className="w-6 h-6 text-green-400" />
              <h2 className="text-xl font-bold text-green-400">
                {language === "hi-IN" ? "एमसीआर संचार कंसोल" : "MCR COMMUNICATION CONSOLE"}
              </h2>
              <div
            ...
            {/* Voice Input Section */}
            <div className="bg-gray-800 border-4 border-green-400/30 rounded-lg p-6 mb-6">
              <h3 className="text-green-400 text-lg font-bold mb-4">
                {language === "hi-IN" ? "🎙️ आवाज़ इनपुट" : "🎙️ VOICE INPUT"}
              </h3>
              <SpeechToTextButton onTranscript={handleVoiceTranscript} language={language} disabled={!isConnected} />
            </div>

            {/* Manual Text Input Section */}
            <div className="bg-gray-800 border-4 border-blue-400/30 rounded-lg p-6 mb-6">
              <h3 className="text-blue-400 text-lg font-bold mb-4">
                {language === "hi-IN" ? "✍️ मैनुअल टेक्स्ट इनपुट" : "✍️ MANUAL TEXT INPUT"}
              </h3>
              <div className="space-y-4">
                <input
                  ...
                  placeholder={language === "hi-IN" ? "यहाँ अपना संदेश टाइप करें..." : "Type your message here..."}
                ...
                >
                  ...
                  {language === "hi-IN" ? "टेक्स्ट संदेश भेजें" : "SEND TEXT MESSAGE"}
                </button>
              </div>
            </div>

            {/* Last 3 Messages Sent */}
            <div className="bg-gray-800 border-4 border-gray-600 rounded-lg p-6 flex-1">
              <h3 className="text-gray-400 text-lg font-bold mb-4">
                {language === "hi-IN" ? "भेजे गए अंतिम 3 संदेश" : "LAST 3 MESSAGES SENT"}
              </h3>
              <div className="space-y-3 max-h-48 overflow-y-auto">
                {sentMessages.map((msg) => (
                  <div
                    ...
                    className={`border-2 rounded p-3 ${
                      msg.type === "emergency" ? "bg-red-900 border-red-600" : "bg-gray-900 border-gray-600"
                    }`}
                  >
                    <div className="text-xs text-gray-400 mb-2 flex items-center gap-2">
                      {msg.type === "emergency" && <span className="text-red-400">🚨</span>}
                      {formatTime(msg.timestamp)}
                    </div>
                    <p className={`text-sm font-mono ${msg.type === "emergency" ? "text-red-300" : "text-gray-300"}`}>
                      {msg.text}
                    </p>
                  </div>
                ))}
                {sentMessages.length === 0 && (
                  <div className="text-center text-gray-500 py-8 text-lg">
                    {language === "hi-IN" ? "अभी तक कोई संदेश नहीं भेजा गया" : "No messages sent yet"}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="bg-black p-6">
          <div className="h-full flex flex-col">
            <div className="flex items-center gap-3 mb-6">
              <div className={`w-6 h-6 rounded-sm ${isConnected ? "bg-green-400 animate-pulse" : "bg-red-400"}`}></div>
              <h2 className="text-xl font-bold text-green-400 font-mono tracking-wider">
                {language === "hi-IN" ? "इंजन रूम संदेश डिस्प्ले" : "ENGINE ROOM MESSAGE DISPLAY"}
              </h2>
            </div>

            {/* Main LED Display */}
            <div
              className={`border-4 rounded-lg mb-6 flex-1 p-6 ${
                emergencyAlert ? "bg-red-900 border-red-400" : "bg-black border-green-400"
              }`}
            >
              <div className="h-full flex flex-col justify-center">
                {currentMessage ? (
                  <div className="text-center">
                    <div
                      className={`font-mono text-3xl lg:text-5xl leading-relaxed tracking-wider mb-6 min-h-[120px] flex items-center justify-center border-4 rounded p-6 ${
                    ...
                    <div
                      className={`font-mono text-lg mb-6 ${
                        currentMessage.type === "emergency" ? "text-red-400/70" : "text-green-400/70"
                      }`}
                    >
                      FROM: {currentMessage.source} | TIME: {formatTime(currentMessage.timestamp)}
                    </div>
                    {currentMessage.type !== "emergency" && (
                      <button
                  ...
                  <div className="text-center text-green-400/50 font-mono text-2xl lg:text-3xl">
                    {language === "hi-IN" ? "आने वाले संदेश का इंतज़ार..." : "WAITING FOR INCOMING MESSAGE..."}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-900 border-4 border-green-400/30 rounded-lg p-6">
              ...
              <div className="space-y-3 max-h-40 overflow-y-auto">
                {receivedMessages.map((msg) => (
                  <div
                    key={msg.id}
                    className={`border-2 rounded p-3 ${
                      msg.type === "emergency" ? "bg-red-900 border-red-400/30" : "bg-black border-green-400/30"
                    }`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span
                        className={`text-xs font-mono ${
                          msg.type === "emergency" ? "text-red-400/70" : "text-green-400/70"
                        }`}
                      >
                        {msg.type === "emergency" && "🚨 "}
                        {formatTime(msg.timestamp)}
                    ...
                    <p className={`text-sm font-mono ${msg.type === "emergency" ? "text-red-300" : "text-green-300"}`}>
                      {msg.text}
                    </p>
                  </div>
                ))}
                {receivedMessages.length === 0 && (
                  <div className="text-center text-green-400/50 py-4 font-mono text-lg">
                    {language === "hi-IN" ? "कोई संदेश प्राप्त नहीं हुआ" : "NO MESSAGES RECEIVED"}
                  </div>
                )}
              </div>
            </div>
