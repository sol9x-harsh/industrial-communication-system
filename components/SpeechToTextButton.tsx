...
export function SpeechToTextButton({
  ...
  const initializeSpeechRecognition = useCallback(() => {
    ...
    if (!SpeechRecognition) {
      ...
      if (browser === "Firefox") {
        setError("Firefox doesn't support Web Speech API. Please use Chrome, Edge, Safari, or Brave.")
      } else {
    ...
    try {
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        switch (event.error) {
          ...
          case "not-allowed":
            if (browser === "Brave") {
              errorMessage = "Microphone blocked by Brave. Click the shield icon and allow microphone access."
            } else {
  ...
  const requestMicrophonePermission = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (browserInfo === "Brave") {
        setError("Microphone access denied in Brave. Click the shield icon in address bar and allow microphone access.")
      } else {
  ...
  }, [isSupported, isListening, language, requestMicrophonePermission, transcript])
    ...
    <div className="text-xs text-amber-400 bg-amber-400/10 border border-amber-400/30 rounded p-3 mt-2">
      ...
      <div className="space-y-2">
        ...
        <ol className="list-decimal list-inside space-y-1 text-xs">
          <li>
            Type <code className="bg-amber-400/20 px-1 rounded">brave://settings/privacy</code> in address bar
          </li>
          <li>
            Set "Block fingerprinting" to <strong>Standard</strong> (not Strict)
          </li>
          <li>
            Go to <code className="bg-amber-400/20 px-1 rounded">brave://settings/content/microphone</code>
          </li>
          <li>Add this site to "Allow" list</li>
          <li>Refresh this page</li>
        </ol>
        <p className="text-amber-300">Or click the shield icon 🛡️ in your address bar and allow microphone access.</p>
      </div>
    </div>
  )
      ...
      <div className="flex flex-col">
        ...
        {browserInfo === "Firefox" && (
          <div className="text-xs text-red-400 bg-red-400/10 border border-red-400/30 rounded p-3 mt-2">
            <strong>Firefox Not Supported:</strong> Please use Chrome, Edge, Safari, or Brave browser for speech
            recognition.
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="text-xs text-gray-400 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          Browser: {browserInfo} | Language: {language === "hi-IN" ? "हिंदी" : "English"}
        </div>
        {confidence > 0 && <div className="text-green-400">Confidence: {Math.round(confidence * 100)}%</div>}
      </div>
          ...
          <div className="bg-gray-800 border-2 border-green-400/30 rounded p-2 min-h-[40px] text-xs font-mono">
            {error ? (
              ...
              <div className="text-green-400 animate-pulse">
                🎤 {language === "hi-IN" ? "सुन रहा है... अब बोलें!" : "Listening... Speak now!"}
              </div>
            ) : (
              <div className="text-gray-400">{language === "hi-IN" ? "माइक दबाएं और बोलें" : "Tap mic to speak"}</div>
            )}
          </div>
        </div>
      </div>
